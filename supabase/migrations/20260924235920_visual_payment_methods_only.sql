...
UPDATE public.business_payment_methods
SET is_enabled = false
WHERE method_code NOT IN ('CASH', 'CARD', 'TRANSFER');

INSERT INTO public.business_payment_methods (business_id, method_code, display_name, is_enabled)
SELECT b.id, m.method_code, m.display_name, true
FROM public.businesses b
CROSS JOIN (VALUES
  ('CASH'::text, 'Efectivo'::text),
  ('CARD'::text, 'Tarjeta'::text),
  ('TRANSFER'::text, 'Transferencia'::text)
) AS m(method_code, display_name)
ON CONFLICT (business_id, method_code)
